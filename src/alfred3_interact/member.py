...
from alfred3.randomizer import SessionGroup
    ...
    def session_data(self) -> dict:
        ...
        if saving_method(self.exp) == "local":
            ...
            query = {
                ...
                "exp_session_id": self.session_id,
                "type": dm.EXP_DATA,
            }
            ...
            if d is None:
                self.mm.log.warning(
                    f"{self} did not find any session data. Trying to recover by waiting."
                )
                start = time.time()
                while d is None:
